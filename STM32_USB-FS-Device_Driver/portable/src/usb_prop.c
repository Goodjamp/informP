...
ONE_DESCRIPTOR Device_Descriptor =
  {
    ...
    sizeof(sDeviceDescriptor)
  };

ONE_DESCRIPTOR Config_Descriptor =
  {
    (uint8_t*)&CustomHID_ConfigDescriptor,
    sizeof(sUSBHIDDescriptor)
  };
...
ONE_DESCRIPTOR String_Descriptor[5] =
  {
    ...
    {(uint8_t*)CustomHID_StringInterface, CUSTOMHID_SIZ_STRING_PRODUCT}
...
void CustomHID_SetConfiguration(void)
{
  ...
  if (pInformation->Current_Configuration != 0)
  {
    ...
    for(cnt=0;cnt < sizeof(PrevXferComplete)/sizeof(PrevXferComplete[0]); cnt++)
    	PrevXferComplete[cnt] = 1;
