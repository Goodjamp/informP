//#include <stddef.h>

//#include "FreeRTOS.h"
//#include "task.h"
//#include "queue.h"
//#include "timers.h"
//#include "processing_USART.h"
//#include "stm32f10x_usart.h"
//#include "stm32f10x_tim.h"

#include "funct.h"

const uint8_t auchCRCHi[] = {

		 };

const uint8_t auchCRCLo[] = {  };

uint16_t CRC16(u8 *puchMsg, u16 usDatalen) {
	unsigned uIndex;
	uint8_t uchCRCHi = 0xFF;
	uint8_t uchCRCLo = 0xFF;
	while (usDatalen--) {
		uIndex = uchCRCHi ^ *puchMsg++;
		uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex];
		uchCRCLo = auchCRCLo[uIndex];
	}
	return (uchCRCHi << 8 | uchCRCLo);
}

//---------------функция modus_add_CRC---------------
// функция modus_add_CRC - дописывает значение контрольной суммы CRC в конец строки
// входные аргументы:
// p_sent - указатель на начало строки для которой нужно рассчитать контрольную сумму
// length_sent - полный размер строки (с учетом полей контрольной суммы) в байтах
void modus_add_CRC(u8 *p_sent, u16 length_sent) {
	u16 res_CRC16;
	res_CRC16 = CRC16(p_sent, length_sent - 2);
	Convert_ShortTooCharHiLo(res_CRC16, (p_sent+length_sent-2));
}

//**********************************************************************
//-------------ПРЕОБРАЗОВАНИЕ ЧИСЛОВЫХ ТИПОВ ДАННЫХ--------------------*
////**********************************************************************


//------------функция ShortTooCharHiLo_-------------------------------
// функция ShortTooCharHiLo_ - преобразует число типа u16 в масив u8
// порядок следования чисел в масиве: [0]Hi,[1]Lo
void Convert_ShortTooCharHiLo_(U_short_to_char my_short_to_char,u8 *buffer){        // first Hi Second Lo

	(*buffer)=my_short_to_char.a_char[1];
	buffer++;
	(*buffer)=my_short_to_char.a_char[0];
}

//------------функция ShortTooCharLoHi_-------------------------------
// функция ShortTooCharHiLo_ - преобразует число типа u16 в масив u8
// порядок следования чисел в масиве: [0]Lo,[1]Hi
void Convert_ShortTooCharLoHi_(U_short_to_char *my_short_to_char,u8 *buffer) 		// first Lo Second Hi
	{
	(*buffer)=my_short_to_char->a_char[0];
	buffer++;
	(*buffer)=my_short_to_char->a_char[1];
}



//------------функция Convert_CharTooShortLoHi_-------------------------------
// функция CharTooShortHiLo_ - преобразует масив двух чисел типа u8 в число типа u16
// порядок следования чисел в масиве: [0]Lo,[1]Hi
void Convert_CharTooShortLoHi_(U_short_to_char *my_short_to_char,u16 *buffer) 		// first Lo Second Hi
	{
	(*buffer)=my_short_to_char->u_short;
}

//------------функция CharTooShortHiLo_-------------------------------
// функция CharTooShortHiLo_ - преобразует масив двух чисел типа u8 в число типа u16
// порядок следования чисел в масиве: [0]Hi,[1]Lo
void Convert_CharTooShortHiLo_(U_short_to_char *my_short_to_char,u16 *buffer) 		// first Hi Second Lo
	{
	(*buffer)=(my_short_to_char->a_char[0]<<8)|my_short_to_char->a_char[1];
}




void IntTooChar(unsigned int *a,unsigned char *buffer)
{
		 unsigned int b;
		unsigned char *Msg;
		Msg=buffer;
		b=*a;
	*Msg = (unsigned char)b;
	Msg++;
	b=b>>8;
	*Msg = (unsigned char)b;
	Msg++;
	b=b>>8;
	*Msg = (unsigned char)b;
	Msg++;
	b=b>>8;
	*Msg = (unsigned char)b;
}
void UintTooChar(unsigned int *a,unsigned char *buffer)
{

}
unsigned char IntTooOneUChar(int *a)
{
		unsigned int *b;
		unsigned char res;

		b=(unsigned int*)a;
		res = (unsigned char)(*b);

		return res;
}
void FloatTooChar(float *a,unsigned char *buffer)
{
	unsigned char *b;

	b=(unsigned char*)((void*)(a));
	*(buffer+0)=*(b+0);
	*(buffer+1)=*(b+1);
	*(buffer+2)=*(b+2);
	*(buffer+3)=*(b+3);
}
int CharTooInt(unsigned char *buffer)
{
	int b;
	unsigned char ach[4];
	ach[0]=*buffer;
	ach[1]=*(buffer+1);
	ach[2]=*(buffer+2);
	ach[3]=*(buffer+3);
	//printf("\n buffer0=%x buffer1=%x \n ",*(buffer),*(buffer+1));
	b=(((unsigned int)ach[3])<<24)+(((unsigned int)ach[2])<<16)+(((unsigned int)ach[1])<<8)+
	(unsigned int)ach[0];
	//printf("\n    b======%x \n",b);
	return b;
}
unsigned int CharToouint(unsigned char *buffer)
{
	unsigned int b;
	unsigned char ach[4];
	ach[0]=*buffer;
	ach[1]=*(buffer+1);
	ach[2]=*(buffer+2);
	ach[3]=*(buffer+3);
	//printf("\n buffer0=%x buffer1=%x \n ",*(buffer),*(buffer+1));
	b=(((unsigned int)ach[3])<<24)+(((unsigned int)ach[2])<<16)+(((unsigned int)ach[1])<<8)+
	(unsigned int)ach[0];
	//printf("\n    b======%x \n",b);
	return b;
}
float	CharToofloat(unsigned char *buffer)
{
	float	*b;
	b=(float*)buffer;
	return *b;
}

unsigned short CharTooushort(unsigned char *buffer)
{
	unsigned short b;
	unsigned char ach[2];
	ach[0]=*buffer;
	ach[1]=*(buffer+1);
	b=(((unsigned short)ach[1])<<8)+(unsigned short)ach[0];
	return b;
}
unsigned short CharTooshort(unsigned char CharHi,unsigned char CharLo) // first Hi second Lo
	{ unsigned short b;
		b=(((unsigned short)CharHi)<<8)+(unsigned short)CharLo;
		return b;
	}

unsigned short CharToosshort(unsigned char CharHi,unsigned char CharLo) // first Hi second Lo
	{ short b=0;
		b=b|((((unsigned short)CharHi)<<8)|(unsigned short)CharLo);
		return b;
	}

//-----------------u8 get_array_bit-----------------
// get_array_bit - take bit value from data array
// input arg:
// data_in - pointer on array
// num_bit - number bit position
// output arg:
// bit value - 0 or 1.
u8 get_array_bit(u8* data_in, u16 num_bit){
	u8 byte_pos=num_bit/8;
	u8 bit_pos=num_bit-byte_pos*8;
	if(*(data_in+byte_pos)&(1<<bit_pos))
	{
		return 1;
	}
	else{
		return 0;
	}

}

